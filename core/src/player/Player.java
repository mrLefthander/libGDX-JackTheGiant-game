...
public class Player extends Sprite {
    ...
    void createBody() {
        ...
        shape.setAsBox((getWidth() / 2f - 18) / GameInfo.PPM,
                (getHeight() / 2f - 10) / GameInfo.PPM);

        FixtureDef fixtureDef = new FixtureDef();
        fixtureDef.density = 5f; // this is the mass
        fixtureDef.friction = 3f; // to not slide
    ...
    public void movePlayer(float x) {
       ...
       // body.setLinearVelocity(x, body.getLinearVelocity().y);
        //body.applyLinearImpulse(x, body.getLinearVelocity().y, body.getPosition().x, body.getPosition().y, true);
       body.applyForceToCenter(x, body.getLinearVelocity().y, true);

       if(body.getLinearVelocity().x >= 2f){
            body.setLinearVelocity(2, body.getLinearVelocity().y);
        } else if(body.getLinearVelocity().x <= -2f) {
            body.setLinearVelocity(-2, body.getLinearVelocity().y);
        }
    }
